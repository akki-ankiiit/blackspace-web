  ...
  return (
    <div style={{ background:T.paper, color:T.ink, fontFamily:F.body, minHeight:'100vh' }}>
      ...
      <section style={{ padding:'60px 40px 40px', position:'relative' }}>
        <div style={{ display:'flex', justifyContent:'space-between', fontSize:11, letterSpacing:'0.2em', textTransform:'uppercase', marginBottom:32, opacity:0.7 }}>
          <span>✳ Selected Work 2023—2025</span>
          <span>46 projects · 18 clients · 1 studio</span>
        </div>

        <div style={{ position:'relative' }}>
          <SPStatusSticker T={T} F={F} top={-20} right={200} rotate={-6} variant="torn" bg={lilac} icon="✎" iconBg={T.popB} label="Just shipped" name="Okinawa v2" />
          <SPStatusSticker T={T} F={F} top={140} right={40} rotate={8} variant="tape" tape={T.popA} bg={mint} icon="★" iconBg={T.ink} label="Award" name="AdFest Silver" />
          <SPStar size={70} color={T.popA} rotate={18} top={30} right={520} />

          <h1 style={{ fontFamily:F.display, fontSize:'clamp(140px, 18vw, 260px)', lineHeight:0.84, fontWeight:700, letterSpacing:'-0.05em', margin:0 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:20, marginTop:40, alignItems:'start' }}>
      ...
      <section style={{ padding:'20px 40px 80px' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(12,1fr)', gridAutoRows:140, gap:14 }}>
          {projects.map((p,i)=>(
            <a key={i} href="#" style={{
              gridColumn:`span ${p.col}`, gridRow:`span ${p.row}`, background:p.bg,
              ...
              <div>
                <div style={{ fontFamily:F.display, fontWeight:700, letterSpacing:'-0.025em', lineHeight:1.02, fontSize: p.big?64:28 }}>{p.title}</div>
      ...
      <section style={{ padding:'100px 40px', background:T.paper }}>
        ...
        <div style={{ borderTop:`1px solid ${T.ink}` }}>
          <div style={{
            display:'grid', gridTemplateColumns:'80px 1.2fr 2fr 1.5fr 1fr 40px',
          ...
          {past.map((r,i)=>(
            <div key={i} style={{
              display:'grid', gridTemplateColumns:'80px 1.2fr 2fr 1.5fr 1fr 40px',
              gap:20, padding:'24px 0', borderBottom:`1px solid ${T.ink}22`,
              alignItems:'baseline',
            }}>
              ...
              <span style={{ textAlign:'right', fontSize:18 }}>↗</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding:'120px 40px', background:lilac, borderTop:`1px solid ${T.ink}`, borderBottom:`1px solid ${T.ink}`, position:'relative', overflow:'hidden' }}>
        <SPStar size={100} color={T.popB} rotate={18} top={40} right={80} />
        <SPStar size={60} color={T.popA} rotate={-12} bottom={60} left={100} />
        <div style={{ maxWidth:1100, margin:'0 auto', textAlign:'center' }}>
          ...
          <p style={{ fontFamily:F.display, fontSize:64, fontWeight:500, letterSpacing:'-0.025em', lineHeight:1.1, margin:0 }}>
      ...
      <section style={{ padding:'100px 40px', textAlign:'center' }}>
        <h2 style={{ fontFamily:F.display, fontSize:'clamp(100px, 14vw, 200px)', lineHeight:0.85, fontWeight:700, letterSpacing:'-0.05em', margin:0 }}>
