    ...
    <div style={{ background:T.paper, color:T.ink, fontFamily:F.body, minHeight:'100vh' }}>
      ...
      <section style={{ padding:'60px 40px 40px', position:'relative' }}>
        ...
        <div style={{ position:'relative' }}>
          <SPStatusSticker T={T} F={F} top={-10} left={80} rotate={-7} variant="tape" tape={T.popC} bg={butter} icon="☕" iconBg={T.ink} label="Right now" name="Brewing coffee" />
          <SPStatusSticker T={T} F={F} top={120} right={180} rotate={6} variant="torn" bg={mint} icon="♫" iconBg={T.popA} label="Office audio" name="Lofi + dogs" />
          <SPStar size={60} color={T.popB} rotate={15} top={40} right={80} />

          <h1 style={{ fontFamily:F.display, fontSize:'clamp(140px, 18vw, 260px)', lineHeight:0.84, fontWeight:700, letterSpacing:'-0.05em', margin:0, textAlign:'center' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1.2fr 1fr', gap:40, marginTop:80, alignItems:'start' }}>
      ...
      <section style={{ padding:'100px 40px', background:T.ink, color:T.paper }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:40 }}>
          {[beliefs.slice(0,3), beliefs.slice(3)].map((col, ci) => (
            <div key={ci}>
              {col.map((b,i)=>(
                <div key={i} style={{ display:'grid', gridTemplateColumns:'80px 1fr', gap:24, padding:'32px 0', borderTop:`1px solid ${T.paper}33` }}>
      ...
      {/* Timeline */}
      <section style={{ padding:'100px 40px', background:butter, borderTop:`1px solid ${T.ink}`, borderBottom:`1px solid ${T.ink}` }}>
      ...
      {/* CTA */}
      <section style={{ padding:'120px 40px', textAlign:'center', background:lilac, borderTop:`1px solid ${T.ink}`, position:'relative', overflow:'hidden' }}>
        <SPStar size={100} color={T.popA} rotate={15} top={40} right={100} />
